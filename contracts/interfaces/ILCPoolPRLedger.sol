...
interface ILCPoolPRLedger {
  ...
  function poolToId(address token0, address token1) external view returns(uint256);
  function setPoolToId(address token0, address token1, uint256 poolId) external;
